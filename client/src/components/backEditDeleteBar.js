...
export default function BackEditDeleteButtons(props) {
    ...
    const [loginModalNextAction, setLoginModalNextAction] = useState({action: 'home'});
    ...
    async function deleteRecipe(recipeId) {
        const response = await fetch(`https://ethans-cookbook.herokuapp.com/api/${recipeId}`, {
            method: "DELETE",
            body: JSON.stringify({
    ...
    return (
        <div className="back-edit-delete-container">
            ...
            <button className="delete-edit-fake-button" onClick={() => {handleEditClick()}}>
                <u>
                    edit
                </u>
            </button>
            <button className="delete-edit-fake-button" onClick={() => {handleDeleteClick()}}>
                <u>
                    delete
                </u>
            </button>
            <AreYouSureDeleteModal show={showDeleteModal} recipeId={props.recipeId} deleteRecipe={deleteRecipe} closeModal={closeDeleteModal}/>
            <LoginModal show={showLoginModal} closeModal={closeLoginModal} userStatus={props.userStatus} setUserStatus={props.setUserStatus} nextAction={loginModalNextAction}/>
        </div>
    );
}
